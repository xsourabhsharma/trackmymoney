  ...
  if (charges.length === 0) {
    return (
      <div className="p-6 rounded-2xl bg-[var(--bg-base)] border border-[var(--border-light)] flex flex-col h-full items-center justify-center min-h-[250px] text-center">
        ...
        <p className="text-[10px] text-[var(--text-muted)] max-w-[200px]">You have no active subscription renewals coming up.</p>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--text-muted)] font-medium">Next 5 renewals</p>
          </div>
        </div>
        <button className="text-[10px] font-bold text-[var(--text-muted)] hover:text-[var(--text-main)] uppercase tracking-widest flex items-center gap-1 transition-colors">
            ...
            <div key={charge.id} className={`flex items-center justify-between p-3 rounded-xl border border-[var(--border-light)] hover:bg-[var(--bg-muted)] transition-colors group ${alertClass}`}>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-[var(--bg-base)] border border-[var(--border-light)] flex flex-col items-center justify-center shadow-sm">
                  <span className="text-[8px] font-bold text-[var(--text-muted)] uppercase tracking-widest leading-none mb-0.5">{format(dateObj, 'MMM')}</span>
                ...
                <div>
                  ...
                  <p className="text-[9px] text-[var(--text-muted)] uppercase tracking-widest">{daysAway === 0 ? 'Today' : `In ${daysAway} days`}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[12px] font-light text-[var(--text-main)]">${Number(charge.amount).toFixed(2)}</p>
                <p className="text-[9px] text-[var(--text-muted)] uppercase tracking-widest">{charge.interval}</p>
              </div>
            </div>
          )
        })}
