...
function StatusBadge({ pct, isOver }: { pct: number; isOver: boolean }) {
  if (isOver) return <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-red-50 text-red-600 border border-red-100 uppercase tracking-tight">Over Budget</span>
  if (pct >= 80) return <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-orange-50 text-orange-600 border border-orange-100 uppercase tracking-tight">Near Limit</span>
  if (pct >= 50) return <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-blue-50 text-blue-600 border border-blue-100 uppercase tracking-tight">Comfortable</span>
  return <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-green-50 text-green-600 border border-green-100 uppercase tracking-tight">On Track</span>
}
  ...
  if (categoryBudgets.length === 0) {
    return (
      <div className="py-20 text-center flex flex-col items-center gap-5 border-2 border-dashed border-[var(--border-light)] rounded-[24px] bg-[var(--bg-surface)]/30">
        ...
        <button
          onClick={onAddBudget}
          className="flex items-center gap-2 px-5 py-2.5 bg-[var(--text-main)] text-[var(--bg-base)] rounded-full text-[10px] font-bold uppercase tracking-widest shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all btn-press"
        >
