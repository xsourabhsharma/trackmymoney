...
export function AIAdvisorCard({ stats, lastInsight }: Props) {
  ...
  return (
    <div className="bg-[var(--bg-surface-dark)] rounded-[24px] p-6 text-[var(--text-inverse)] flex flex-col justify-between shadow-xl min-h-[300px]">
      <div className="flex items-center justify-between mb-4 pb-4 border-b border-white/10">
        <div>
          <h3 className="text-sm font-bold uppercase tracking-widest flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-white/80" /> AI Advisor
          </h3>
          {timeAgo && (
            <span className="text-[9px] text-white/40 font-medium mt-1 block">Updated {timeAgo}</span>
          )}
        </div>
        <button 
          onClick={handleRefresh}
          disabled={isLoading}
          className="px-3 py-1.5 bg-white/10 hover:bg-white/20 transition-all rounded-full text-[10px] font-bold border border-white/5 uppercase tracking-wider flex items-center gap-1.5 disabled:opacity-50"
        >
      ...
      <div className="flex-grow">
        {isLoading ? (
          <div className="w-full h-full flex flex-col items-center justify-center text-center py-10 gap-3">
            ...
            <p className="text-[10px] text-[var(--text-inverse-muted)] font-bold uppercase tracking-widest">Crunching numbers...</p>
          </div>
        ) : error ? (
          ...
          <div className="flex flex-col gap-4">
            {insights.map((insight, idx) => (
                <div key={insight.id || idx} className="flex flex-col gap-1.5 p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors">
                    ...
                    <div className="text-[9px] font-bold text-white/40 uppercase tracking-tighter italic mt-1">💡 {insight.actionHint}</div>
                </div>
            ))}
          </div>
        ) : (
          <div className="w-full h-full flex flex-col items-center justify-center text-center py-10 gap-3">
            ...
            <p className="text-xs text-[var(--text-inverse-muted)] leading-relaxed font-medium max-w-[250px]">
