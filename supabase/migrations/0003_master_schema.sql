-- Track My Money Master Schema (Aligned strictly with Drizzle schema.ts)
...
CREATE INDEX "idx_transactions_user_category_date" ON "public"."transactions" ("user_id", "category_id", "date");
...
CREATE INDEX "idx_subscriptions_user_next_charge" ON "public"."subscriptions" ("user_id", "next_charge_date");
CREATE INDEX "idx_subscriptions_user_merchant" ON "public"."subscriptions" ("user_id", "merchant");
...
-- 6. Generate Policies explicitly
DO $$
DECLARE
    t_name text;
BEGIN
    FOR t_name IN 
        SELECT unnest(ARRAY[
            ...
            'subscriptions', 'subscription_events', 'health_snapshots', 
            'chat_messages', 'debts', 'ai_insights', 'import_jobs', 'import_rows'
        ])
    LOOP
        EXECUTE format('CREATE POLICY "Users can view own data" ON "public".%I FOR SELECT USING (auth.uid() = user_id);', t_name);
        EXECUTE format('CREATE POLICY "Users can insert own data" ON "public".%I FOR INSERT WITH CHECK (auth.uid() = user_id);', t_name);
        EXECUTE format('CREATE POLICY "Users can update own data" ON "public".%I FOR UPDATE USING (auth.uid() = user_id);', t_name);
        EXECUTE format('CREATE POLICY "Users can delete own data" ON "public".%I FOR DELETE USING (auth.uid() = user_id);', t_name);
    END LOOP;
END $$;

-- Drop standard user category view and create a broader one that allows reading system default categories (where user_id is null)
DROP POLICY IF EXISTS "Users can view own data" ON "public"."categories";
CREATE POLICY "Users can view own data or default categories" ON "public"."categories" FOR SELECT USING (user_id IS NULL OR auth.uid() = user_id);
...
-- 7. Trigger to provision 'Main Account' and User Profile on Auth insertion
-- This automatically creates an entry in profiles and a default 'Main Account' so AI inserting transactions doesn't fail.
...
BEGIN
  ...
  -- Insert a default account so AI tool and manual additions work immediately
